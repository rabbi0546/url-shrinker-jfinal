...
public class Mapper {
	 ...
	 * 当前redis服务器的Cache对象
	 */
	private static Cache CACHE = Redis.use(SysContext.REDISPLUGINNAME);
	 ...
	 * @return url的id（小写）
	 */
	private String idGenerater(int len, int count){
	 ...
	 * @param url 要添加的url地址，恶意用户也许会输入id，但是还是会被过滤掉的
	 * @param remoteHost 请求进行这个操作的那台电脑的IP
